...
public class LoginForAdmin extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp)throws ServletException, IOException {
		...
		try {
			Admin admin=Admin.find(studentID, password); //查找是否有匹配的数据
			session.setAttribute("admin", admin); //保存数据到session中，以便在前台获取到数据
		...
		out.println("<html><script>alert('学号或密码错误！');location.href='adminLogin.jsp'; </script></html>");
		out.close();
	}
		}
	}
